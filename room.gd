...
func visualize(c, i, j, disp, global_scaling):
	...
	temp.add_point(p2)
	temp.width = w
	temp.default_color = Color.WHITE_SMOKE
	...
	if corridors[0]:
		...
		t.width = 60
		t.default_color = Color.SKY_BLUE
		c.root.add_child(t);
	if corridors[1]:
		t = Line2D.new()
		...
		t.width = 60
		t.default_color = Color.LIGHT_BLUE
		c.root.add_child(t);
	if corridors[2]:
		t = Line2D.new()
		t.add_point(Vector2(i * global_scaling + disp[0], j * global_scaling + disp[1] + (width+30)/2 + 5))
		t.add_point(Vector2(i * global_scaling + disp[0], j * global_scaling + disp[1] + (width+30)/2))
		t.width = 60
		t.default_color = Color.DEEP_SKY_BLUE
		c.root.add_child(t);
	if corridors[3]:
		t = Line2D.new()
		...
		t.width = 60
		t.default_color = Color.CORNFLOWER_BLUE
		c.root.add_child(t);
