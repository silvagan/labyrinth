...
func _ready():
	...
	$"../Camera2D".position = Vector2(-220,-100)
...
func generate_chunk(x, y):
	...
	$"../NavigationRegion2D".add_child(node_root)
	...
	initialize_table(c)
	chunks[new_key] = c
	draw_grid(grid_width, grid_height, Vector2(x, y))
	debug_chunk_border(grid_width, grid_height, Vector2(x, y))
...
func _on_timer_timeout():
	$"../NavigationRegion2D".BakeNavigationMesh(true);
